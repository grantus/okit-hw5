...
public final class ServerStorage implements IAccountDataSource, IAuthorizationSource, AutoCloseable {
    ...
    private OperationResponse testSession(String login, Long session) {
        if (login == null || session == null) {
            return OperationResponse.NULL_ARGUMENT_EXCEPTION;
        }

        try (Connection conn = source.getConnection()) {
            return validateSession(conn, login, session);
        } catch (SQLException e) {
            return new OperationResponse(OperationResponse.UNDEFINED_ERROR, e.getMessage());
        }
    }
